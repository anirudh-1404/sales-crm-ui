...
const SidebarLink = ({ to, icon: IconComp, label }) => (
    <NavLink to={to}
      ...
      ${isActive ? "bg-green-600 text-white shadow-md shadow-green-200" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"}`
        }>
        {({ isActive }) => (
            <>
            ...
            <aside className={`${sidebarOpen ? "w-64" : "w-0 overflow-hidden"} flex-shrink-0 bg-white border-r border-gray-200 flex flex-col transition-all duration-300`}>
                ...
                <div className="px-4 pt-4 pb-2">
                    <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                        ...
                        <span className="text-xs font-semibold text-green-700">Sales Representative</span>
                    </div>
                </div>

                <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-5">
                    <div>
                        <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest px-2 mb-2">My Work</p>
                        <div className="space-y-1">
                            <SidebarLink to="/rep/dashboard" icon={LayoutDashboard} label="My Dashboard" />
                            <SidebarLink to="/rep/deals" icon={Briefcase} label="My Deals" />
                            <SidebarLink to="/rep/companies" icon={Building2} label="My Companies" />
                            <SidebarLink to="/rep/contacts" icon={ContactRound} label="My Contacts" />
                        </div>
                    </div>
                </nav>
            ...
            <div className="flex-1 flex flex-col overflow-hidden min-w-0">
                <header className="h-16 bg-white border-b border-gray-200 flex items-center px-5 gap-4 flex-shrink-0">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 flex-1 max-w-xs pl-3 pr-4 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg text-gray-400 hover:border-green-300 hover:bg-green-50/30 transition text-left"
                    >
                        <Search size={15} />
                    ...
                    <div className="flex items-center gap-3 pl-3 border-l border-gray-200">
                        <div className="relative">
                            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-green-500 to-green-300 flex items-center justify-center text-white font-bold text-sm shadow">{initials}</div>
                            <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-400 border-2 border-white rounded-full" />
                        </div>
                        <div className="hidden sm:block">
                            <p className="text-sm font-semibold text-gray-800 leading-none">{user ? `${user.firstName} ${user.lastName}` : "Rep Name"}</p>
                            <p className="text-xs text-gray-400 mt-0.5">{user?.email || "Sales Representative"}</p>
                        </div>
                    </div>
                </header>
                ...
                <footer className="h-10 bg-white border-t border-gray-100 flex items-center justify-between px-6 text-xs text-gray-400 flex-shrink-0">
                    <span>Copyright &copy; <span className="text-green-500 font-medium">mbdConsulting</span></span>
